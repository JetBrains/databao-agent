...
class LighthouseAgent(BaseAgent):
    ...
    def render_system_prompt(self) -> str:
        ...
        db_schema = SimpleDuckDBAgenticExecutor.describe_duckdb_schema(self._data_collection.get_connection())
    ...
    def execute(self, messages: list[BaseMessage]) -> ExecutionResult:
        ...
        try:
            for chunk in self._compiled_graph.stream(
                init_state, stream_mode="values", config=RunnableConfig(recursion_limit=50)
            ):
